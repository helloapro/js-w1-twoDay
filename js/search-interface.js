    ...
    $.get("https://www.goodreads.com/search/index.xml?key=" + goodReadsKey + "&q=" + searchString).then(function(response){
      ...
      parseString(stuff, function(err, result){
        ...
        if(works){
          ...
          for(var i = 0; i < 5; i++){
            $("#searchResults").append("<div class='row'><div class='col-md-1 book'><img class='img-responsive center-block' src='" + works[i].best_book[0].small_image_url[0] + "'> </div><div class='col-md-3 book book-main'>Title: <em>" +
            ...
            works[i].best_book[0].id[0]._ + "</span></div><div class='col-md-8 book'></div></div>");
          }
        } else {
